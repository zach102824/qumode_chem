...
--------------------------------------------------------------
  ...
  map          Jordan–Wigner, 8 qubits → two qumodes, L=16 each
  SNAP block   S D on each qumode, then BS(β, φ); α~U(-3,3), angles ~U(0,π)
...
Ansatz
------
  one layer = (ECD block + SNAP block)
  circuit   = [(ECD+SNAP)]^L  then ECD
...
def _safe_fun(x, h, n_layers, nfock):
    try:
        return hybrid_energy_and_grad(x, h, n_layers, nfock)
    except RuntimeError:
        return 1.0e6, np.zeros_like(x, dtype=float)


def self_check(nfock: int = 4) -> None:
    """Small-cutoff FD check so the script fails fast if the ansatz is broken."""
    rng = np.random.default_rng(0)
    n_layers = 1
    x = hybrid_random_params(n_layers, nfock, rng)
    dim = nfock * nfock
    h = np.diag(np.linspace(-2.0, 1.0, dim)).astype(np.complex128)
    e, g = hybrid_energy_and_grad(x, h, n_layers, nfock)
    worst = 0.0
    for i in (0, 7, 8, 11, 12):
        xp = x.copy()
        xp[i] += 1e-6
        fd = (hybrid_energy(xp, h, n_layers, nfock) - e) / 1e-6
        rel = abs(fd - g[i]) / (abs(g[i]) + 1e-8)
        worst = max(worst, rel)
        if rel > 0.10:
            raise RuntimeError(f"grad slot {i}: fd={fd:.4e} analytic={g[i]:.4e} rel={rel:.3f}")
    print(f"self-check ok (nfock={nfock}, worst FD rel err {worst:.2e})", flush=True)
...
def run_geometry(
    ...
    for n_layers in layers:
        npar = n_params(n_layers, nfock)
        print(f"-- r={r:.2f}  (ECD+SNAP)^{n_layers} + ECD   npar={npar} --", flush=True)
        trials = []
        for seed in seeds:
            rng = np.random.default_rng(seed + 17 * n_layers)
            if init == "grow" and n_layers > layers.start and seed in prev_best_x:
                x0 = hybrid_grow_params(prev_best_x[seed], n_layers - 1, nfock, rng)
            else:
                x0 = hybrid_random_params(n_layers, nfock, rng)
            res = _optimize(x0, lambda x, hh=h, nl=n_layers: _safe_fun(x, hh, nl, nfock), maxiter, tol)
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("data/results/h4_hybrid.json"))
    ...
    result = {
        "settings": {
            ...
            "ansatz": "[(two-mode ECD then SNAP+BS)]^L then two-mode ECD, project q=|g>",
